...
> **2026-09-26 개정:** 이 그룹들은 이제 `docs/06` §9.1의 워크스페이스 아래에 배치된다. 관리·감사와 모니터링은 **운영 콘솔**, VOC 분류·담당 배정·처리는 **피드백 공간**(FeedbackOps), 분석 공간의 공지·VOC에는 사용자용 화면(공지, 내 VOC 등록·상태 확인, 설문 응답, 가공 상태 조회)만 남는다. 아래 목록의 기능 자체는 유효하며 공간 배치만 이 개정을 따른다.
