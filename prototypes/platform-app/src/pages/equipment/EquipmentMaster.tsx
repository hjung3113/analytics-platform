...
export default function EquipmentMaster() {
  ...
  const source = usePlatformQuery(signal => serve({ role, global, signal, compute: ({ equipment }) => equipment }), null, scope.status === 'valid');
  ...
  return <PlatformPage description={ko ? '설비 속성 → 유효구간 → Audit. 합성 데이터 · 조회 전용 Candidate.' : 'Equipment attributes → validity → audit. Synthetic data · read-only Candidate.'}>
    <QueryView query={source}>{rows => <PlatformDataTable<Equipment>
      ...
      loadPage={(page, signal) => serve({ role, global, signal, compute: ({ equipment }) => sortAndPage(filterEquipment(equipment, q, status, maker), page), isEmpty: data => data.total === 0 })}
