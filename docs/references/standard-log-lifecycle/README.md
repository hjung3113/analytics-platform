...
공통 Navigation의 그룹 구조는 유지한다(2026-09-26 개정: `docs/06` §9.1 기준 분석 공간 6그룹, 관리·감사는 운영 콘솔). 각 카테고리에 접기/펼치기 chevron, 하위 메뉴 들여쓰기, 현재 페이지 표시를 둔다. 하단에는 즐겨찾기·최근방문, 헤더에는 Scope·메뉴 검색·사용자 메뉴를 둔다.
...
분석 공간 그룹(06 §9.1)의 확장형 사이드바를 적용한 랜딩 화면. 즐겨찾기, 최근 방문, 접근 가능한 메뉴 바로가기를 중심에 둔다. 불필요하게 큰 KPI 카드나 장식용 차트로 채우지 않는다.
