...
| D6 | `mock/world.ts` → `kernel/registry`(`Permission` 타입) | mock이 Kernel 런타임 모듈에 의존 | `Permission`을 `contracts`로 |
...
| --- | --- | --- | --- | --- |
| `@ap/contracts` | Kernel 계약 | URL 계약(`kernel/url.ts`), manifest 타입(`MenuEntry`의 선언부, `ContextKey`, `Capability`, `PageType`, `Permission`), 응답·Trust envelope(`ApiResponse`, `Trust`, `Assessment`, `Outcome`), `AuditEvent`, `Text`(i18n 문자열 쌍) | 없음 | 없음 |
| `@ap/ui` | 공통 컴포넌트(UI Primitive) | `styles/tokens.css`, `ui/components/shadcn/*`, `Button`, `cn`, `StatusBadge`(`Tone`) | 있음 | 외부 라이브러리만 |
| `@ap/kernel` | Kernel 기능 | `PlatformProvider`/`usePlatform`/`PlatformLink`, `usePlatformQuery`, i18n Provider, Registry 런타임(`createRegistry`, `matchRoute`, `pathFor`), `PlatformAdapter` 포트, 슬롯 등록 | 있음 | `contracts` |
...
| `@ap/mock-server` | (개발용) | `mock/world`, `mock/server`, `mock/jobs`와 테스트. `PlatformAdapter` mock 구현 | 없음 | `contracts` |
...
5. `contracts`는 React·브라우저 API에 의존하지 않는다(향후 백엔드와 공유할 수 있게).
...
export type PlatformAdapter = {
  ...
  contextOptions(scopeId: string): Promise<ContextOptions>;    // GlobalContextBar 조건 축 선택지·설비 풀
...
- `matchesCondition` 같은 조건 판정은 서버 책임이다. 셸은 어댑터가 준 결과(해당 설비 수 등)만 표시한다.
...
`manifests`의 각 항목은 현재 `MenuEntry`와 같은 선언(06 §5)을 담고, 화면은 `component: lazy(() => import('./pages/X'))`로 지연 로드한다. 앱은 다음처럼 조립한다.
...
| 테스트 | Vitest를 패키지별로. 기존 51개 테스트는 원래 파일과 같이 이동 | |
...
2. **contracts 추출:** D3·D6·D7 타입 이동. 런타임 변화 없음.
...
5. **메뉴 패키지:** 그룹별로 `menus/*`로 이동, `api.ts` 도입(D8), Registry를 manifest 등록 방식으로(D1).
6. **경계 lint와 생성기:** 규칙 켜고 CI에 추가, `gen:menu`로 빈 메뉴 하나를 만들어 검증한 뒤 삭제.

이 순서를 마치면 D1–D8이 모두 해소되고, 워크스페이스 층(06 §9.1)은 Registry `space` 필드와 셸 공간 전환기로 이 구조 위에 얹는다.
