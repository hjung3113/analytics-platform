...
지표 버전을 고정해도 늦게 완료된 occurrence, 마스터 이력 정정, `module_class_map` 재분류로 같은 URL의 숫자는 바뀔 수 있다. 딥링크는 "조회조건과 지표 버전의 재현"만 보장하고 "같은 숫자의 재현"은 보장하지 않는다 — 결과 화면에는 계산 기준시각을 표시한다. 상세는 `04_frontend_ui_ux.md`의 딥링크 절.
