...
TZ를 몰라도 단일 설비 또는 동일 wall-clock 기준이 확인된 설비 집합의 naive 조회는 허용한다. `R` 부재 시에도 독립적으로 유효한 `defaultRangeTo`는 물질화할 수 있고 자동 재집계만 보류한다. 기본 구간은 06 §6.3, 원천 진행 경계 `R`/자동 창 `H`의 상세 정책은 [05 지연 완료 허용 시간](05_roadmap_and_open_questions.md#지연-완료-허용-시간-decided--정책-메커니즘--구체-숫자)을 참조한다.
