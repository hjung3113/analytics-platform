...
| 공간 | 대상 | 그룹 / 화면 |
| --- | --- | --- |
| 분석 | 모든 사용자 | 운영 개요 / 설비관리 / 기준정보관리 / 생산성 분석 / 지표관리 / 공지·VOC(사용자용: 공지, 내 VOC 등록·상태 확인, 설문 응답, 가공 상태 조회 — FeedbackOps 1단계에서 등록·응답 제출은 원본 화면으로 이동, [저장소 연결 문서](integration/repository-layout.md#feedbackops-통합-방식-decided-2026-09-26)) |
...
- **Registry:** 메뉴는 소속 공간을 선언한다(필드명 Candidate: `space`). 위 표의 **공간별 배치**는 Decided다. 공간별 그룹 상한 숫자와, 표 항목 중 무엇이 그룹이고 무엇이 그 아래 화면인지의 구분은 **Open**이다([인터뷰 기록](reviews/2026-09-26-workspace-ops-interview.md#남은-open)). 새 그룹은 공간 추가보다 먼저 기존 그룹에 흡수할 수 있는지 검토한다.
