...
└── 공지·VOC                          [사용자용 화면만, 2026-09-26]
    ├── 공지 목록/상세
    ├── 내 VOC 등록·상태 확인, 설문 응답   (FeedbackOps 원천, 사용자에게 보이는 상태만. 1단계 등록·응답 제출은 원본 화면으로 이동)
    └── 가공 상태 조회                     (설비·기간별 단계 상태 + 원인 분류 + VOC로 문의, 06 §19)

[운영 콘솔 공간]                        개발자·운영자만, 06 §9.1
├── 시스템 모니터링                       (적재 워커 보고 기반, 01 가공 상태 보고)
├── 파이프라인 트레이스 (개발자용)         (원시 오류·단계 로그·재처리)
├── 메뉴 활용률
└── 관리·감사                           [분석 공간에서 이동]
    ├── 권한/역할 관리 (메뉴×데이터 스코프)
    └── 변경 감사 Audit Trail (전역 뷰 — 각 상세 화면에도 탭으로 노출)

[피드백 공간]                           내부 담당자, FeedbackOps 단계적 통합
└── VOC 분류 · Finding · Task Request/Task · Milestone
```
...
위 사이드바는 **분석 공간** 기준이다. 접근 가능한 공간이 2개 이상인 사용자에게는 로고 영역이 공간 전환기가 되고, 운영 콘솔·피드백 공간은 자기 그룹만 사이드바에 표시한다(06 §9.1).
