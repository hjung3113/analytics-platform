...
Scope 선택은 추상적인 접근 범위 선택이다. 사이트/공장/라인 고정 계층을 뜻하지 않는다. 헤더의 Scope와 분석 필터는 시각적으로 구분하되 둘 다 계약에 따른 URL 상태를 소비한다.
...
- 요청 Scope, 현재 검증 상태, 사용자가 접근 가능한 Scope 선택 항목을 읽는다(§6.2). 구체 계층 API는 Open이다.
...
- 시간 표시/입력은 §6.3을 따른다. 구간 경계와 시간대 정책을 이 화면에서 확정하지 않는다.
