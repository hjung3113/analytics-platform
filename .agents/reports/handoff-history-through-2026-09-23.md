...
기준 커밋 `897bf34`의 HANDOFF 전체를 보존한다. 상대 링크만 저장소 원래 대상으로 보정했다. 당시 Open·미커밋·착수 안내는 현재 상태가 아니다. 현행 진입점은 [HANDOFF](../../HANDOFF.md)와 [INDEX](../../docs/INDEX.md)다. 본문 속 코드 표기 경로는 저장소 루트 기준이다.

---

# Handoff — 2026-09-23 M3 커밋 / M4 데이터 정책 이관 완료

M3는 `c09c101`로 커밋했다. 사용자 요청에 따라 M4의 05 세 정책 상세→01 이관을 수행했고 M4 이관과 후속 정렬은 이 HANDOFF 갱신을 포함한 커밋으로 묶는다(해시는 `git log -1`로 확인). push는 하지 않았다. 아래 과거 기록의 상태 안내보다 이 상단과 실제 Git 상태를 우선한다.

- 현재 상세 원본: [01 데이터 운영 정책](../../docs/01_architecture_and_data_contract.md#데이터-운영-정책). 05에는 결정 상태와 기존 링크 안내가 남는다. 06은 전역 시간 계약 원본을 유지한다.
- 변경: 01/02/03/05/06/INDEX/REQUIREMENTS, DESIGN, integration 후보 문서와 이 HANDOFF·[M4 기록](../../.agents/reports/m4-data-policy-migration-2026-09-23.md). 세 정책 본문 문자열 보존, 링크 87개, 기존 제목 유지, diff 공백 검사를 통과했다. 제품 런타임/별도 모델 검토는 하지 않았다.
- M4의 선택한 이관 단위는 완료했다. 전체 문서 축약·파일 삭제는 완료했다고 주장하지 않는다. DESIGN 분할은 필요성이 확인되지 않아 보류했다.
- 후속 정렬 완료: 01 Scope 요약, DESIGN preset 안내, integration 후보2 인증 문구를 현행 원본에 맞췄다. 상속/초기 자동 기간/SSO 프로토콜 Open과 후보2 보류는 유지한다. M5는 실제 제품 구현 요청이 있을 때 코드·테스트 증거를 연결한다.
- 앞으로 GPT 계열은 GPT-6를 사용한다(Luna/Sol 포함, GPT-5.6 사용 안 함). 실제 제공 모델을 확인하고 과거 실행 모델명은 보존한다.
- 다음 Grok 호출은 4.7(가용성 확인 필요). 과거 4.6 기록 보존. FeedbackOps 및 독립 parser 경계와 역사 보고서 보존.

---

## 이전 기록 — 당시 상태

# Handoff — 2026-09-22 M2 커밋 / M3 요구사항 상태 정리 완료

## 현재 상태

사용자 요청 “M2 커밋하고 M3 진행”을 수행했다. **M2는 `d20efe0`으로 커밋**, M3는 완료했으나 **미커밋**이다. push하지 않았다. 아래 과거 기록의 M2 미커밋/M3 미착수 안내는 이 상단으로 대체한다. 다음 세션은 AGENTS·실제 Git 상태부터 확인하고 현재 변경을 보존한다.

M3 변경: [PLATFORM_REQUIREMENTS](../../PLATFORM_REQUIREMENTS.md)의 결정/후보/미결과 구현 증거를 구분하고 H·인증·Scope 등 낡은 요약을 원본에 정렬했다. 모델 표수와 107개 체크 상태, 기존 완료 두 줄, 15개 질문은 보존했다. 원본 계약·제품 코드·폴더 구조·FeedbackOps gitlink는 변경하지 않았다.

검증: 로컬 링크/fragment 31개, 체크·출처 보존 검사, diff 공백 검사 통과. 대표 4개 문서 워크스루와 범위/한계는 [M3 실행 기록](../../.agents/reports/m3-requirements-state-2026-09-22.md)에 있다. M3 별도 모델 토론·제품 런타임 시험은 하지 않았다.

## 다음 작업

- M1 `ce4687f`, M2 `d20efe0` 완료/커밋. M3 완료/미커밋. M4 이후는 미실행이다.
- M4는 사용자가 원한 “필요한 문서만 필요한 내용으로 남기기”를 실제 본문 정리에 적용할 단계다. [이행 계획](../../.agents/reports/doc-operations-2026-09-22/migration-plan.md)의 원본 이관 후보를 현재 파일에 재대조하고 가장 작은 묶음부터 진행한다. 문단 대응·참조 갱신·예외 보존 없이 파일을 합치거나 삭제하지 않는다.
- DESIGN preset 문구와 integration 후보2 OIDC 표현은 알려진 후속 정정 후보다. 제품 미결 답변을 만들어 채우지 않는다. M5 코드/테스트 연결과 M6 운영 평가도 미실행이다.
- 사용자 최신 모델 선호: 앞으로 Grok은 **4.7**, 4.6은 사용하지 않는다. 호출 시 실제 가용성을 확인한다. 과거 실행 이력의 4.6 표기는 변경하지 않는다.
- 독립 FeedbackOps gitlink `b5dd614ac8da3792cb1627e7daeffb8fc9c4944e`와 parser upstream 경계를 유지한다. 역사 보고서/sample/validation.json은 덮어쓰지 않는다.

---

## 이전 M2 세션 기록 — 당시 상태이며 현재 실행 지시 아님
